  ...
  try {
    ...
    if (Array.isArray(countRes) && countRes[0]?.count === 0) {
      const seeds = [
        ...
        { step_number: "Step 04", icon: "🛡️", title: "Finish & Garansi", desc: "Serah terima pekerjaan dengan jaminan kerapian maksimal, garansi ketahanan produk, dan kepuasan pelanggan.", sort_order: 4, is_active: 1 }
      ];
...
export async function PUT(req: Request) {
  try {
    ...
    await executeQuery(
      "UPDATE workflow_steps SET step_number = ?, icon = ?, title = ?, description = ?, sort_order = ?, is_active = ? WHERE id = ?",
      [
        step_number || "Step 01",
        icon || "💬",
        title,
