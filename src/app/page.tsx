          ...
          {featuredProducts.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {featuredProducts.slice(0, 8).map((product: any) => {
                ...
                const currentPrice = activeVariant.discount_price && activeVariant.discount_price > 0
                  ? activeVariant.discount_price
                  : activeVariant.price || 0;
                const originalPrice = activeVariant.price || 0;
                const hasDiscount = activeVariant.discount_price && activeVariant.discount_price > 0 && activeVariant.discount_price < originalPrice;

                return (
                  <div
                    key={product.id}
                    className="group flex flex-col justify-between p-5 rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/80 hover:border-brand-orange/40 hover:shadow-xl transition-all duration-300"
                  >
                    <div>
                      {/* Image preview box */}
                      <div className="w-full h-48 rounded-2xl border border-zinc-100 dark:border-zinc-800/80 bg-zinc-50 dark:bg-zinc-800/50 mb-4 overflow-hidden relative flex items-center justify-center">
                        {hasDiscount && (
                        ...
                        {activeVariant.image_url ? (
                          ...
                          <div className="text-center p-4">
                            ...
                            <div className="text-[10px] text-zinc-400 uppercase tracking-wider font-bold">Variasi {activeVariant.color || "Standar"}</div>
                          </div>
                        )}
                        
                        {/* Spec Tier Badge */}
                        {product.tier && (
                          <span className="absolute top-3 right-3 text-[9px] font-bold tracking-wider uppercase px-2.5 py-1 rounded bg-white dark:bg-zinc-900 text-brand-orange border border-zinc-200/30 shadow-sm z-10">
                            {product.tier}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Title */}
                      <h4 className="text-base font-bold text-brand-charcoal dark:text-white group-hover:text-brand-orange transition-colors">
                        {product.name}
                      </h4>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {variants.map((v: any, idx: number) => (
                              <span
                                key={idx}
                                title={v.color}
                                className="px-2 py-1 text-[10px] font-semibold bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-md border border-zinc-200/50 dark:border-zinc-700/50"
                              >
                                {v.color}
                              </span>
                            ))}
                    ...
                    <div className="mt-6 pt-4 border-t border-zinc-100 dark:border-zinc-800/80 flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-base font-extrabold text-brand-orange">
                          {currentPrice > 0 ? `Rp ${currentPrice.toLocaleString("id-ID")}` : "Konsultasi"}
                        </span>
                      </div>

                      <Link
                        href="/katalog"
                        className="flex items-center gap-1 bg-zinc-900 hover:bg-brand-orange text-white text-xs font-bold px-3.5 py-2 rounded-xl transition-all shadow-sm"
                      >
